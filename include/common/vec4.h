#pragma once
/******************************************************************************
 * A four components vector (templated component type, meant to be arithmetic)
 *****************************************************************************/
...
template <typename T> inline T norm(const TVec4<T> a);

/* Static Helpers */

template <typename T> const TVec4<T> TVec4<T>::Zero{ 0, 0, 0 };

/* Functions implementations */

template <typename T>
inline constexpr TVec4<T>::TVec4(T x, T y, T z, T w)
	: x{ x }
	, y{ y }
	, z{ z }
	, w{ w }
...
template <typename T>
inline constexpr TVec4<T>::TVec4(const TVec3<T> &xyz, T w)
	: xyz{ xyz }
	, w{ w }
{
}
...
template <typename T>
inline TVec4<T>::TVec4(const T *t)
	: x{ t[0] }
	, y{ t[1] }
	, z{ t[2] }
	, w{ t[3] }
...
template <typename T> inline T &TVec4<T>::operator[](int n)
{
	assert(n >= 0 && n <= 3);
	return (&x)[n];
}
...
template <typename T> inline TVec4<T> &TVec4<T>::operator-() const
{
	return { -x, -y, -z, -w };
}

template <typename T> inline TVec4<T> &TVec4<T>::operator+=(const TVec4<T> &a)
{
	x += a.x;
	y += a.y;
	z += a.z;
...
template <typename T> inline TVec4<T> &TVec4<T>::operator*=(const T &t)
{
	x *= t;
	y *= t;
	z *= t;
