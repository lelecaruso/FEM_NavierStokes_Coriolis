#pragma once
/******************************************************************************
 * Quat : Quaternions, mostly used for their ability to cheaply and 
 *        conveniently represent rotations in 3D.
 *****************************************************************************/
...
template <typename T> inline TQuat<T> operator*(const TQuat<T> &a, const T &t);

template <typename T> inline TQuat<T> operator*(const T &t, const TQuat<T> &a);
...
template <typename T>
inline constexpr TQuat<T>::TQuat(T x, T y, T z, T w)
	: x{ x }
	, y{ y }
	, z{ z }
	, w{ w }
...
template <typename T>
inline constexpr TQuat<T>::TQuat(TVec3<T> xyz, T w)
	: xyz{ xyz }
	, w{ w }
{
}
...
template <typename T> inline TQuat<T> &TQuat<T>::operator*=(const T &t)
{
	xyz *= t;
	w *= t;
	return (*this);
...
template <typename T> inline T dot(const TQuat<T> &a, const TQuat<T> &b)
{
