...
int is_visible(const float* vtx, int n, const float* pvm)
{
  ...
  for (int i = 0; i < n; ++i)
  {
    W = pvm[3] * vtx[3 * i + 0] + pvm[7] * vtx[3 * i + 1] + pvm[11] * vtx[3 * i + 2] + pvm[15];
    if (clipx)
    {
      T = pvm[0] * vtx[3 * i + 0] + pvm[4] * vtx[3 * i + 1] + pvm[8] * vtx[3 * i + 2] + pvm[12];
      clip_R = clip_R && (T > W);
      clip_L = clip_L && (T < -W);
      clipx  = clip_R || clip_L;
    }
    if (clipy)
    {
      T = pvm[1] * vtx[3 * i + 0] + pvm[5] * vtx[3 * i + 1] + pvm[9] * vtx[3 * i + 2] + pvm[13];
      clip_T = clip_T && (T > W);
      clip_B = clip_B && (T < -W);
      clipy  = clip_T || clip_B;
    }
    if (clipz)
    {
      T = pvm[2] * vtx[3 * i + 0] + pvm[6] * vtx[3 * i + 1] + pvm[10] * vtx[3 * i + 2] + pvm[14];
      clip_F = clip_F && (T > W);
      clip_N = clip_N && (T < -W);
      clipz  = clip_F || clip_N;
    }
